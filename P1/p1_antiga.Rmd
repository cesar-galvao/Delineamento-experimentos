...
tabela_arvores %>%
  ...
  knitr::kable(
    ...
    longtable = TRUE,
    linesep = "",
    col.names = c("Árvores", "Data 1", "Data 2", "Data 3", "Data 4"),
    escape = FALSE
    ) %>%
  kableExtra::kable_styling(
      position = "center",
...
pvalor_variancias <- var.test(tabela_arvores$D1, tabela_arvores$D2) %>% tidy() %>%
  pull(p.value)
```
Considerando $H_0$ sendo a igualdade entre as variâncias, obtém-se p-valor `r round(tabela_arvores$D1,3)`. Ou seja, não se rejeita a hipótese de igualdade de variâncias.
